...
- Out:
  ...
  - A web dashboard, database, or new dependency.
...
- Generated Markdown does not include environment variables, Chrome profile paths, remote debugging ports unless already part of normal smoke output, or save strings.
- `npm run check`, `./melvor-report.js --help`, and a single-character dry run are executed and documented at closeout.
...
- request-Running `journal all --record` creates or appends one Markdown file per configured character under `journal/`. -> This backlog slice. Proof: The command refuses unknown character names the same way other single-character commands do, or clearly reports the load failure without partial writes for that character.
